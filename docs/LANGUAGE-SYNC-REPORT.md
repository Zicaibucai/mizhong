...
日期 2026-09-18 ｜ 起点 `6a96b44` ｜ 终点 `ff1aeef`（本地 main，**未推送**）
...
| `npm test` | **196 通过 / 0 失败**（起点是 111） |
| `npm run build` | 通过（`/admin/sync` 已生成，`/sitemap.xml` 为 1 小时 ISR） |

新增 85 个测试，覆盖需求第十四节里可自动化的部分：首次全语言同步、只改一个字段就只翻那一个、
...
1. **8 个提交还没有推到 origin**（本地 main 领先 origin/main 9 个提交）。
   部署命令是 `git pull --ff-only origin main`，所以推不推、什么时候推由你定。
2. **首次全站同步尚未执行**（第 11 节）。建议先 `--limit 2` 跑一条验证 Key 与模型连通。
3. **3 条未发布的测试商品**（`product_3`、`product_3_copy`、`product_3_copy_2`）
   仍未处理 —— 未获授权我不动它们。
4. **严格回退的开启时机**：第三阶段验证通过后再置 `STRICT_LOCALE_FALLBACK=true`。
5. **是否让发布在 DeepSeek 不可用时也能放行？** 现在按需求实现成「任何必要语言失败
   则发布整体失败」，好处是不会出现中英混杂的版本，代价是 DeepSeek 挂掉时
   管理员连改一个错别字都发不出去。要我加一个显式的「跳过翻译直接发布」应急开关吗？

### 尚未完成

6. **页面区块的新增/删除**：页面编辑器目前只能编辑已有区块，不能新增或删除。
   数据层（`applyPageDraftToLive`）已经支持增删，缺的是界面。这是改造前就有的限制。
7. **按内容类型配置「发布哪些语言」**：现在固定是除中文外的全部 10 种。
   引擎与任务都已经能接受任意语言子集，缺的是设置项。
8. **同步中心的进度不自动刷新**：推进过程中进度条会更新，但底下的内容列表要
   手动点「刷新」。自动轮询没做，是为了避免在翻译进行中反复查十几条内容的同步计划。
9. **`TranslationState` 的清理**：删商品、恢复版本会清；但直接删库或未来新增的
