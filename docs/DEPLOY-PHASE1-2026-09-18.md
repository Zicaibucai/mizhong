...
## 12. 追加修复二：联系方式只显示一条、微信永不显示（`8617e67`）

11.1 修掉空串遮蔽之后，邮箱确实出来了 —— 但**只出来一个**，微信还是不显示。
根因是另外三处独立的问题叠在一起：

1. `CHANNEL_ORDER` 被当成**筛选**用：每种类型只取第一条，而且只认
   WhatsApp / Email / Phone，微信连名单都不在。现在它是**排序** ——
   后台有几条显示几条，同类型内保持后台的 sortOrder。
2. 取不到内容就用 `PUBLIC_CONTACTS` 兜底顶上，于是页面出现内置的过时联系方式 ——
   这正是「看起来像缓存」的真正来源。兜底现在只在该类型**一条内容都没有**时补位。
3. 微信号、地址**没有可点链接**（`deriveContactHref` 不处理它们），会被整条丢掉。
   这类内容不是「没有值」，只是点不了 —— 现在照常显示、渲染成不可点的文字。

为此加了共用的 `ChannelLink`（按有没有链接决定渲染 `<a>` 还是 `<span>`），
页脚、侧边联系轨道、询盘区三处共用。顺带修 `contact-rail` 的图标查找 ——
它按 `key` 查，而 key 只可能是那三种，**后台一加微信就会取到 undefined 直接崩**。

## 13. `NEXT_PUBLIC_SITE_URL` 改为域名

`http://47.238.7.93` → `http://htd123.com`。

`htd123.com` 目前**只有 HTTP**（HTTPS 握手不通），所以规范地址用 `http://` 而不是
`https://` —— 指向一个访问不了的协议比指向 IP 好不了多少。等配上证书再改成 https。

改后 canonical、`og:url`、hreflang、sitemap 全部指向域名：

```
<link rel="canonical" href="http://htd123.com/zh/products/tehsugouzi"/>
hrefLang="en" href="http://htd123.com/en/products/tehsugouzi"
```

## 14. 追加修复后的验证（2026-09-18 17:09 部署，`8617e67`）

| 项 | 结果 |
| --- | --- |
| 备份 | `mizhong-20260918-170922.dump`，141,882 字节，34 张表 |
| 页脚联系方式 | 3 个邮箱 + WhatsApp + 电话 + 微信，**全部显示** |
| 微信 `Sofa_Materials_Mia` | 出现（不可点文字） |
| 商品 URL（4 个商品 × 10 语言） | **40 / 40 = 200** |
| 各语言首页（11 种） | 全部 200 |
| canonical / hreflang / sitemap | 全部指向 `htd123.com` |
| 本次部署后新增错误 | **0**（错误日志最后一条仍是 09-17 18:46） |

> 部署顺序已按第 11.2 节的结论执行：`pm2 stop → rm -rf .next → npm run build → pm2 start`。
> 构建产物在启动前检查过 —— 联系方式与 canonical 都是对的，没有再次被旧代码污染。
